-- =============================================================================
-- GESTION360 - OPTIMISATION DES STATISTIQUES PLATEFORME V3
-- Harmonise le décompte des agences actives en incluant les statuts NULL (actifs par défaut).
-- =============================================================================
...
BEGIN
    ...
    mrr_calc AS (
        ...
        WHERE COALESCE(subscription_status, 'active') IN ('active', 'trial')
    )
    SELECT 
        jsonb_build_object(
            ...
            'activeAgencies', (SELECT count(*) FROM public.agencies WHERE COALESCE(subscription_status, 'active') IN ('active', 'trial')),
...
COMMENT ON FUNCTION public.get_platform_stats_v2 IS 'Agrégation haute performance des statistiques pour le Dashboard Super Admin (V3 avec COALESCE).';

GRANT EXECUTE ON FUNCTION public.get_platform_stats_v2() TO authenticated;
