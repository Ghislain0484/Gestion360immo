// Dans supabase.ts - Remplace getDashboardStats
async getDashboardStats(agencyId: string): Promise<DashboardStats> {
  try {
    ...
    // Vérifications d'erreurs (maintenant receiptsError est accessible)
    ...
    return {
      ...
      monthlyRevenue,  // number, pas RentReceiptSummary (fix TS2322)
  ...
  try {
    ...
    for (let i = 0; i < months; i++) {
      ...
      const monthData = Array.isArray(receipts)
        ...
        : [];
      const revenue = monthData.reduce((sum: number, r: RentReceiptSummary) => sum + (r.total_amount || 0), 0);  // Fix TS2769
      result.push({
        month: monthNames[month - 1],
        revenue,
        commissions: revenue * 0.1,
      });
    }
