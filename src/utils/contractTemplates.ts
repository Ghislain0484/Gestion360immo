...
`;
  ...
  static async generateManagementContractForOwner(
    ...
    const contractData = {
      ...
      agencyRegister: agencyData.commercialRegister,
      ownerFirstName: ownerData.firstName || ownerData.first_name,
      ownerLastName: ownerData.lastName || ownerData.last_name,
      ownerAddress: ownerData.address,
      ownerPhone: ownerData.phone,
      ownerEmail: ownerData.email,
      propertyTitle: this.getPropertyTitleLabel(ownerData.propertyTitle || ownerData.property_title),
      propertyTitleDetails: ownerData.propertyTitleDetails || ownerData.property_title_details,
      commissionRate,
    ...
    return {
      ...
      terms: this.generateManagementContract(contractData),
      documents: [],
    };
  }

  // Générer contrat de location automatiquement
  static async generateRentalContractForTenant(
    tenantData: any,
    agencyData: any,
    propertyData?: any,
    rentalTerms?: {
    ...
    const defaultRent = 350000; // Loyer par défaut
    const defaultDeposit = defaultRent * 2; // 2 mois de caution
    const defaultDuration = 12; // 12 mois
    ...
    const contractData = {
      ...
      agencyEmail: agencyData.email,
      agencyRegister: agencyData.commercialRegister,
      tenantFirstName: tenantData.firstName || tenantData.first_name,
      tenantLastName: tenantData.lastName || tenantData.last_name,
      tenantAddress: tenantData.address,
      tenantPhone: tenantData.phone,
      tenantEmail: tenantData.email,
      tenantProfession: tenantData.profession,
      tenantNationality: tenantData.nationality,
      propertyTitle: propertyData?.title || 'Bien immobilier à définir',
      propertyAddress: propertyData?.location?.commune || 'Adresse à définir',
      propertyDescription: propertyData?.description || 'Description à compléter',
      monthlyRent: rentalTerms?.monthlyRent || defaultRent,
      deposit: rentalTerms?.deposit || defaultDeposit,
      charges: rentalTerms?.charges,
      startDate: new Date(),
      duration: rentalTerms?.duration || defaultDuration,
    };
    ...
    return {
      ...
      agency_id: agencyData.id,
      start_date: new Date(),
      end_date: new Date(Date.now() + (contractData.duration * 30 * 24 * 60 * 60 * 1000)),
      monthly_rent: contractData.monthlyRent,
      deposit: contractData.deposit,
      charges: contractData.charges,
      commission_rate: 10,
      commission_amount: contractData.monthlyRent * 0.1,
      status: 'draft' as const,
      terms: this.generateRentalContract(contractData),
      documents: [],
    };
  }
  ...
  static printContract(contractData: any, agencyData: any, clientData: any) {
