...
export const OwnerPropertyList: React.FC<OwnerPropertyListProps> = ({
  ...
  >(async (agencyId: string) => {
    ...
    // ---- Map to create `tenant` field from tenants[0] ----
    const mapped = (data ?? []).map((p: any) => ({
      ...p,
      contracts: p.contracts.map((c: any) => ({
        ...c,
        tenant: c.tenants?.[0] || null, // unique locataire
      })),
    }));
  ...
  const totalMonthlyRevenue = properties.filter(p => p.status === 'active').reduce((sum, p) => sum + p.owner_payment, 0);
  const totalCommissions = properties.filter(p => p.status === 'active').reduce((sum, p) => sum + p.commission_amount, 0);
  ...
  return (
    <div className="space-y-6">
          ...
          <Card key={property.id} className="bg-white/80 backdrop-blur-sm border-gray-200 hover:shadow-lg transition-all duration-300">
            <div className="p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <Badge variant={getStatusColor(property.status)} size="sm">{getStatusLabel(property.status)}</Badge>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
                ...
                <div className="text-center p-3 bg-purple-50/80 rounded-lg">
                  ...
                  <p className="font-semibold text-purple-800">{new Date(property.contract_date).toLocaleDateString('fr-FR')}</p>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex space-x-2">
                  <Button variant="ghost" size="sm" onClick={() => { setSelectedProperty(property); setShowDetails(true); }}><Eye className="h-4 w-4" /></Button>
                  <Button variant="ghost" size="sm"><Edit className="h-4 w-4" /></Button>
                  <Button variant="ghost" size="sm"><Printer className="h-4 w-4" /></Button>
                  <Button variant="ghost" size="sm" onClick={() => handleContact(property)}><MessageSquare className="h-4 w-4" /></Button>
                </div>
              </div>
            </div>
          </Card>
      ...
      {/* Modal */}
      <Modal isOpen={showDetails} onClose={() => { setShowDetails(false); setSelectedProperty(null); }} title="Détails du bien" size="lg">
        {selectedProperty && (
          <div className="space-y-6">
