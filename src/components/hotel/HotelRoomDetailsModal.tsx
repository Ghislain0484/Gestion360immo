...
import { User, Calendar, CheckCircle, LogOut, Loader2, Shield, Phone } from 'lucide-react';
...
interface HotelRoomDetailsModalProps {
    room: any;
    onClose: () => void;
    onSuccess: () => void;
}

export const HotelRoomDetailsModal: React.FC<HotelRoomDetailsModalProps> = ({ room, onClose, onSuccess }) => {
    ...
    const fetchDetails = async () => {
        try {
            setLoading(true);
            const data = await dbService.modular.getRoomWithBooking(room.id);
            setDetails(data);
        } catch (error) {
    ...
    const handleCheckout = async () => {
        ...
        try {
            ...
            await dbService.modular.checkoutBooking(details.booking.id, room.id, 'hotel');
            toast.success('La chambre a été libérée. Elle est maintenant en attente de ménage.');
            onSuccess();
            onClose();
        } catch (error) {
            console.error('Error during checkout:', error);
            toast.error('Erreur lors de la libération de la chambre');
        } finally {
    ...
    const handleRecordPayment = async () => {
        ...
        try {
            ...
            const transaction: any = {
                ...
                description: `Paiement Séjour Hôtel - Chambre ${room.room_number}`,
            ...
            ReceiptPrinter.printTransactionReceipt(transaction, client, booking, agencyName, 'Hôtel', room.room_type + ' ' + room.room_number);
            ...
            <html>
                ...
                <body>
                    ...
                    <div class="content">
                        ...
                        <div class="field"><span class="label">Chambre :</span> <span class="value">${room.room_number} (${room.type})</span></div>
    ...
    if (loading) {
        return (
            <div className="p-20 flex flex-col items-center justify-center bg-white rounded-3xl">
                ...
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Récupération des données client...</p>
            </div>
        );
    }
    ...
    if (!details?.booking) {
        return (
            <div className="p-20 text-center bg-white rounded-3xl">
                ...
                <p className="text-sm font-black text-slate-800 uppercase">Aucune occupation active trouvée</p>
        ...
        <Card className="p-0 border-0 shadow-2xl bg-white overflow-hidden max-w-2xl mx-auto rounded-3xl">
            <div className="bg-slate-900 p-8 text-white relative">
                ...
                <div className="flex justify-between items-start relative z-10">
                    <div>
                        <span className="text-[10px] font-black uppercase tracking-[0.2em] text-indigo-400 mb-2 block">Détails Occupation Hôtel</span>
                    ...
                    <section className="space-y-4">
                        ...
                        <div className="space-y-3">
                            ...
                            <div className="flex justify-between items-center p-3 bg-indigo-50 border border-indigo-100 rounded-xl">
                                <span className="text-[10px] font-black text-indigo-600 uppercase">Arrivée Effective</span>
                    ...
                    {isAddingPayment ? (
                        <div className="p-6 bg-amber-50 rounded-2xl border-2 border-amber-100 space-y-4 animate-in fade-in slide-in-from-top-4 duration-300">
                            <div className="flex justify-between items-center">
                                <h4 className="text-[10px] font-black uppercase text-amber-600 tracking-widest italic">Enregistrer un versement (Hôtel)</h4>
                            ...
                            <Button 
                                className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-black italic shadow-lg shadow-indigo-200 border-0"
                        ...
                        <div className="flex items-center justify-between p-6 bg-slate-900 rounded-2xl text-white shadow-xl italic relative overflow-hidden group">
                           <div className="absolute inset-0 bg-gradient-to-r from-indigo-600/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none" />
                            <div>
                                <p className="text-[10px] font-black uppercase opacity-50 tracking-widest">Total du Séjour Hôtel</p>
