...
export const LoginForm: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      // 🔐 Authentifie Supabase (indispensable pour RLS "to authenticated")
      await signIn(email, password);

      // Debug: vérifie qu'on a bien un token après signIn
      const { data: { session }, error: sessErr } = await supabase.auth.getSession();
      if (sessErr) console.warn('auth.getSession error:', sessErr);
      console.log('🔑 after signIn session user?', session?.user?.id ?? null, 'token?', !!session?.access_token);

      // Redirige vers le dashboard (ajuste la route si besoin)
      navigate('/');
    } catch (err: any) {
  ...
  // Laisse ta logique d’inscription agence; on garde un flux simple et compilable
        ...
        <Card>
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            <div className="flex items-center justify-between">
              <div classNam
cat > src/components/auth/LogingForm.tsx <<'TS'
import React, { useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { Building2, Eye, EyeOff } from 'lucide-react';
import { useAuth } from '../../contexts/AuthContext';
